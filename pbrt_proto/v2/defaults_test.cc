...
#include "pbrt_proto/v2/defaults.pb.h"
 ...
 public:
  DefaultsTest() {
    ...
    std::ifstream istream(
        runfiles->Rlocation("_main/pbrt_proto/v2/defaults.binpb"),
...
TEST_F(DefaultsTest, Global) {
  ...
  for (auto& directive : defaults_.global_defaults().directives()) {
    ...
    } else if (directive.has_renderer()) {
      EXPECT_NE(directive.renderer().renderer_type_case(),
                Renderer::RENDERER_TYPE_NOT_SET);
      EXPECT_FALSE(found_renderer);
      found_renderer = true;
...
TEST(Defaults, Renderer) {
  CheckAllFields(Renderer::GetDescriptor(),
                 Defaults::DefaultRenderers::GetDescriptor());
}
